...
Elm.Bricks.make = function (_elm) {
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var displayBrick = function (brick) {
      return brick.hit ? A3($Object.displayObj,brick,A2($Graphics$Collage.rect,0,0),brick.color) : A3($Object.displayObj,
      brick,
      A2($Graphics$Collage.rect,65,10),
      brick.color);
   };
   ...
   var numberOfBricks = $Basics.floor(($Basics.toFloat($Constants.gameWidth) / 60.0 - 1) * 4 - 1);
   var makeBrick = F2(function (x,y) {    return {x: x,y: y,vy: 0,vx: 0,color: $Color.green,hit: false};});
   var initBrick = A2(makeBrick,35 - $Constants.halfWidth,$Constants.halfHeight - 7);
   ...
   var createSubsequentBricks = F3(function (numberOfBricksLeft,brick,bricks) {
      createSubsequentBricks: while (true) {
         ...
         var nextBrick = _U.cmp(brick.x + 66,$Constants.halfWidth - 35) > 0 ? A2(makeBrick,35 - $Constants.halfWidth,brick.y - 11) : A2(makeBrick,
         brick.x + 66,
         brick.y);
...
Elm.Game.make = function (_elm) {
   ...
   var _op = {};
   var outOfPlayArea = function (ball) {    return _U.cmp(ball.y,5 - $Constants.halfHeight) < 0 ? true : false;};
   ...
   var Input = F3(function (a,b,c) {    return {space: a,paddle: b,delta: c};});
   ...
   var defaultGame = {ball: {x: 0,y: 0,vy: -100,vx: 100},state: Pause,player: $Player.makePlayer($Constants.gameHeight),bricks: $Bricks.createBricks};
   var Play = {ctor: "Play"};
...
Elm.Breakout.make = function (_elm) {
   ...
   var txt = function (f) {
      return function (_p0) {
         return $Graphics$Element.leftAligned(f($Text.monospace(A2($Text.color,$Constants.textIvory,$Text.fromString(_p0)))));
      };
   };
   var display = F2(function (_p2,_p1) {
      ...
      A3($Graphics$Collage.collage,$Constants.gameWidth,$Constants.gameHeight,displayObjects));
   });
